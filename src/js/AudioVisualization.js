...
return;
  ...
  let context = new GLOW.Context({
    width: width,
    height: height,
    alpha: false
  });

  context.domElement.classList.add("background");

  $(".main").append(context.domElement);

  context.setupClear( { red: .5, green: .2, blue: .5 } );
  context.GL.enable(context.GL.BLEND);
  context.GL.blendFunc(context.GL.SRC_ALPHA, context.GL.ONE_MINUS_SRC_ALPHA);

  let circlesCount = 256;
  let theta = 0;
  let dTheta = 0.01;
  let multiplier = 1.5;
  let xs = new Float32Array(new Array(circlesCount).fill(0).map((_, i) => i * 20));
  let ys = new Float32Array(circlesCount);
  let phase = new Float32Array(circlesCount);

  let circleShaderInfo = {
    ...
    data: {
      ...
      u_alpha: { value: new Float32Array([.9]) },

      // attributes
      a_color: new Float32Array(Array(circlesCount * 3).fill(1).map(()=> Math.random())),
      a_radius: new Float32Array(Array(circlesCount).fill(10)),
      a_x: new Float32Array(circlesCount),
      a_y: new Float32Array(circlesCount)
    },

    primitives: context.GL.POINTS,
    ...
    usage: {
      a_x: context.GL.DYNAMIC_DRAW,
      a_y: context.GL.DYNAMIC_DRAW
    }
  };

  let shader = new GLOW.Shader(circleShaderInfo);
  ...
  function step() {
    ...
    shader.attributes.a_x.bufferSubData(xs);
    shader.attributes.a_y.bufferSubData(ys);
